...
public class WebAccount implements Account {
    ...
    public function updateUser(userId:String, password:String, token:String):void {
        ...
        try {
            if (this._rememberMe) {
                _local4 = SharedObject.getLocal("TUKGame", "/");
                _local4.data["GUID"] = userId;
                _local4.data["Token"] = token;
                _local4.data["Password"] = password;
                _local4.flush();
            }
        }
        catch (error:Error) {
    ...
    public function verify(verified:Boolean):void {
        this.isVerifiedEmail = verified;
    }
