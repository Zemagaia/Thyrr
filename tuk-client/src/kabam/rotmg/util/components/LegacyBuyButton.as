...
public class LegacyBuyButton extends BuyButton {
    ...
    public static const unholyEssence:BitmapData = IconFactory.makeUnholyEssence();
    public static const divineEssence:BitmapData = IconFactory.makeDivineEssence();
    ...
    public function LegacyBuyButton(prefix:String, size:int, price:int, currency:int, withOutline:Boolean = false, red:Boolean = false) {
        this.staticStringBuilder = new StaticStringBuilder("");
        this.lineBuilder = new LineBuilder();
        super();
        this.prefix = prefix;
        this.text = new TextFieldDisplayConcrete().setSize(size).setColor(red ? 15544368 : 0x363636).setBold(true);
        this.waiter.push(this.text.textChanged);
        var stringBuilder:StringBuilder = prefix != "" ? this.lineBuilder.setParams(prefix, {"cost": price.toString()}) : this.staticStringBuilder.setString(price.toString());
        this.text.setStringBuilder(stringBuilder);
    ...
    override public function setPrice(price:int, currency:int):void {
        ...
        if (((!((this.price == price))) || (!((this.currency == currency))))) {
            ...
            stringBuilder = (((this.prefix) != "") ? this.lineBuilder.setParams(this.prefix, {"cost": price.toString()}) : this.staticStringBuilder.setString(price.toString()));
            this.text.setStringBuilder(stringBuilder);
            this.updateUI();
        }
    }
    ...
    private function updateIcon():void {
        switch (this.currency) {
            ...
            case Currency.FORTUNE:
                ...
                break;
            case Currency.UNHOLY_ESSENCE:
                this.icon.bitmapData = unholyEssence;
                break;
            case Currency.DIVINE_ESSENCE:
                this.icon.bitmapData = divineEssence;
                break;
            default:
    ...
    private function updateBackground():void {
        ...
        GraphicsUtil.drawCutEdgeRect(0, 0, this.getWidth(), this.getHeight(), 0, [0, 0, 0, 0], this.graphicsPath);
    }
    ...
    private function drawOutline(_arg1:Graphics):void {
        ...
        GraphicsUtil.drawCutEdgeRect(0, 0, this.getWidth(), this.getHeight(), 0, GraphicsUtil.NO_CUTS, _local5);
