...
public class Frame extends Sprite {
    ...
    private var titleFill_:GraphicsSolidFill = new GraphicsSolidFill(0x545454, 1);
    private var backgroundFill_:GraphicsSolidFill = new GraphicsSolidFill(0x2B2B2B, 1);
    private var outlineFill_:GraphicsSolidFill = new GraphicsSolidFill(0x7B7B7B, 1);
    ...
    public function addPlainText(key:String, tokens:Object = null):void {
        var field:TextFieldDisplayConcrete;
        var position:Function;
        position = function ():void {
            positionText(field);
            draw();
        };
        field = new TextFieldDisplayConcrete().setSize(12).setColor(0xFFFFFF);
        field.setStringBuilder(new LineBuilder().setParams(key, tokens));
        field.filters = [new DropShadowFilter(0, 0, 0)];
        field.textChanged.add(position);
        addChild(field);
    }

    protected function positionText(displayObject:DisplayObject, _arg2:Boolean = false):void {
        displayObject.y = (this.h_ - (_arg2 ? 66 : 52));
        displayObject.x = INDENT;
        this.h_ = (this.h_ + 20);
    }

    public function addTitle(title:String):void {
        var field:TextFieldDisplayConcrete = new TextFieldDisplayConcrete().setSize(20).setColor(0xB2B2B2).setBold(true);
        field.setStringBuilder(new LineBuilder().setParams(title));
        field.filters = [new DropShadowFilter(0, 0, 0, 0.5, 12, 12)];
        addChild(field);
        field.y = (this.h_ - 60);
        field.x = 15;
        this.h_ = (this.h_ + 40);
    ...
    protected function draw():void {
        ...
        GraphicsUtil.drawCutEdgeRect(-6, -6, this.w_, (20 + 12), 4, [0, 0, 0, 0], this.path1_);
        GraphicsUtil.clearPath(this.path2_);
        GraphicsUtil.drawCutEdgeRect(-6, -6, this.w_, this.h_, 4, [0, 0, 0, 0], this.path2_);
