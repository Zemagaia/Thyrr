...
public class BoostPanel extends Sprite {
    ...
    private function createHeader():void {
        ...
        var _local1:BitmapData = TextureRedrawer.redraw(AssetLibrary.getImageFromSet("interfaceBig", 22), 20, true, 0);
        ...
        _local3.setStringBuilder(new LineBuilder().setParams("Active Boosts"));
    ...
    private function createBackground():void {
        ...
        graphics.drawRect(0, 0, 150, (height + 5));
    ...
    private function addTierBoostIfAble(_arg1:SignalWaiter):void {
        if (this.player.tierBoost) {
            this.tierBoostTimer = this.returnBoostTimer(new LineBuilder().setParams("Tier level increased"), this.player.tierBoost);
    ...
    private function addDropTimerIfAble(_arg1:SignalWaiter):void {
        ...
        if (this.player.dropBoost) {
            _local2 = "1.5x";
            this.dropBoostTimer = this.returnBoostTimer(new LineBuilder().setParams("{rate} drop rate", {"rate": _local2}), this.player.dropBoost);
