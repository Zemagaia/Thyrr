...
public class RankText extends Sprite {
    ...
    public function RankText(numStars:int, largeText:Boolean, showPrefix:Boolean, rank:int = 0, isAdmin:Boolean = false) {
        ...
        if (showPrefix) {
            ...
            this.prefix_.setStringBuilder(new LineBuilder().setParams("Rank: "));
        ...
        mouseEnabled = false;
        mouseChildren = false;
        this.draw(numStars, rank, isAdmin, false);
    }
    ...
    public function draw(numStars:int, rank:int, admin:Boolean, isGameSprite:Boolean):void {
        ...
        var rankText:String = isGameSprite ? "" : this.numStars_.toString() + (rank ? "-" + rank : "");
        text = this.makeText();
        text.setVerticalAlign(TextFieldDisplayConcrete.BOTTOM);
        text.setStringBuilder(new StaticStringBuilder(rankText));
