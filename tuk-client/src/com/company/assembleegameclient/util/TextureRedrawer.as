...
public class TextureRedrawer {
    ...
    public static function redraw(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, glowMult:Number = 1.4):BitmapData {
        ...
        modTex = GlowRedrawer.outline(modTex, glowColor, 0, glowMult, useCache);
        if (useCache) {
    ...
    public static function glowRedraw(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, glowMult:Number = 1.4):BitmapData {
        var hash:* = getHash(size, padBottom, glowColor, sMult, glowMult);
        if (useCache && isCached(tex, hash)) {
            return redrawCaches[tex][hash];
        }
        var modTex:BitmapData = resize(tex, null, size, padBottom, 0, 0, sMult);
        modTex = GlowRedrawer.outlineGlow(modTex, glowColor, 0, glowMult, useCache);
        if (useCache) {
            cache(tex, hash, modTex);
        }
        return modTex;
    ...
    public static function redraw2(tex:BitmapData, size:int, padBottom:Boolean, glowColor:uint, useCache:Boolean = true, sMult:Number = 5, glowMult:Number = 1.4):BitmapData {
        var hash:* = getHash(size, padBottom, glowColor, sMult, glowMult);
        if (useCache && isCached(tex, hash)) {
            return redrawCaches[tex][hash];
        }
        var modTex:BitmapData = resize2(tex, null, size, padBottom, 0, 0, sMult);
        modTex = GlowRedrawer.outline(modTex, glowColor, 0, glowMult, useCache);
        if (useCache) {
            cache(tex, hash, modTex);
        }
    ...
    public static function resize2(tex:BitmapData, mask:BitmapData, size:int, padBottom:Boolean, op1:int, op2:int, sMult:Number = 5):BitmapData {
        if (mask != null && (op1 != 0 || op2 != 0)) {
            tex = retexture(tex, mask, op1, op2);
            size = size / 5;
        }
        var w:int = sMult * size / 100 * tex.width / int(tex.width / 8);
        var h:int = sMult * size / 100 * tex.height / int(tex.width / 8);
        var m:Matrix = new Matrix();
        m.scale(w / tex.width, h / tex.height);
        m.translate(magic, magic);
        var ret:BitmapData = new BitmapDataSpy(w + minSize, h + (padBottom ? magic : 1) + magic, true, 0);
