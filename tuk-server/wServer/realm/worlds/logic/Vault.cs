    ...
    public class Vault : World
    {
        ...
        void InitVault()
        {
            ...
            for (var i = 0; i < _client.Account.VaultCount && vaultChestPosition.Count > 0; i++)
            {
                ...
                var con = new Container(_client.Manager, 0x0403, null, false, vaultChest);
            ...
            while (gifts.Count > 0 && giftChestPosition.Count > 0)
            {
                ...
                if (c < 8)
                    items.AddRange(Enumerable.Repeat(new ItemData(), 8 - c));

                var con = new GiftChest(_client.Manager, 0x0405, null, false);
                con.BagOwners = new int[] { _client.Account.AccountId };
        ...
        public override void LeaveWorld(Entity entity)
        {
            ...
            var x = new StaticObject(_client.Manager, 0x0405, null, true, false, false);
