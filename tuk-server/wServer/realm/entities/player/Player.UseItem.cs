...
namespace wServer.realm.entities
{
    partial class Player
    {
        ...
        public void UseItem(RealmTime time, int objId, int slot, Position pos, byte activateId, int clientTime)
        {
            lock (_useLock)
            {
                ...
                foreach (var stack in Stacks.Where(stack => stack.Slot == slot))
                {
                    ...
                    itemData = ItemData.GenerateData(item.ObjectType);
                    break;
                }
                ...
                if (slot < cInv.Length)
                {
                    ...
                    if (item.Consumable)
                    {
                        ...
                        if (item.SuccessorId != null)
                            successor = ItemData.GenerateData(gameData.IdToObjectType[item.SuccessorId]);
                        ...
                        if (container is GiftChest)
                            if (successor != null)
                            {
                                SendError("Cannot use this item from a gift chest");
                                return;
                            }
                            else
                                db.RemoveGift(Client.Account, itemData, trans);
            ...
            int clientTime, bool isAbility)
        {
            ...
            var tolerance = 0.95;
            var lastUse = _clientLastUse;
            if (activate != null && isAbility &&
                (clientTime > _clientLastUse + _coolDown * tolerance || _clientLastUse == 0))
            {
                if (_clientLastUse == 0) _clientLastUse = clientTime;
                _coolDown = item.Cooldown * 1000 * (100f / (100 + Stats[11]));
                _clientLastUse = clientTime;
            }
            else if (isAbility && (clientTime < _clientLastUse + _coolDown || activate == null))
                return;
        ...
        {
            if (Inventory[1].Item.Power == "Pillar of Flame")
            {
