    ...
    */
    ...
    class GiftCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            var itemData = ItemData.GenerateData(item.ObjectType);
            ...
            // add gift
            var result = player.Manager.Database.AddGift(acc, itemData);
            if (!result)
            {
            ...
            // send out success notifications
            player.SendInfoFormat("You gifted {0} one {1}.", acc.Name, itemName);
