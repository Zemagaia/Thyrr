...
using wServer.networking.packets.incoming.forge;
using wServer.networking.packets.outgoing.forge;
using wServer.realm.entities;

namespace wServer.networking.handlers.forge
{
    class CraftItemHandler : PacketHandlerBase<CraftItem>
    {
        ...
        private void Handle(Client client, CraftItem packet)
        {
            ...
            switch (packet.Id)
            {
                ...
                case 1:
                    ...
                    if (packet.ItemSlot < 4 || packet.ItemSlot > 19)
                    {
                        player.SendError("Invalid item slot.");
                        return;
                    }

                    if (packet.RuneSlot < 4 || packet.RuneSlot > 19)
                    {
        ...
        private void GiveCraftedItem(XmlData gameData, Player player, int[] selectedItems, string id)
        {
            ...
            inventory[slot] = ItemData.GenerateData(objType);
