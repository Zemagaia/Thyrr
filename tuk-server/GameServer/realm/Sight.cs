...
namespace GameServer.realm
{
    public class Sight
    {
        private static readonly Logger Log = LogUtil.GetLogger(typeof(Sight));
        ...
        public HashSet<IntPoint> GetSightCircle(int blocked = 0)
        {
            ...
            var specPlayer = player.SpectateTarget as Player;
            if (specPlayer != null)
            ...
            //}
            //Log.Info(time.ElapsedMilliseconds.ToString());
            return _sCircle;
        }
        ...
        private void CalcUnblockedSight(Wmap map)
        {
            ...
            foreach (var p in unblockedView)
            {
                _ip.X = LastX + p.X;
                _ip.Y = LastY + p.Y;
        ...
        private void CalcBlockedRoomSight(Wmap map)
        {
            ...
            var sPoints = SurroundingPoints;
            var visibleTilesList = _visibleTilesList;
            List<IntPoint> tilesToAdd = new();

            for (var i = 0; i < visibleTilesList.Count; i++)
            {
                var tile = visibleTilesList[i];
        ...
        private void CalcBlockedLineOfSight(Wmap map)
        {
            ...
            foreach (var ray in rays)
                for (var i = 0; i < ray.Value.Length; i++)
                {
                    ...
                    foreach (var sPoint in sPoints)
                    {
                        var intPoint = new IntPoint(sPoint.X + _ip.X, sPoint.Y + _ip.Y);
                        if (map.Contains(intPoint))
                            _sCircle.Add(intPoint);
                    }
                }
        }
