...
namespace GameServer.realm
{
    ...
    {
        private static readonly Logger Log = LogUtil.GetLogger(typeof(ConnectManager));
        ...
        private void Connect(ConInfo conInfo)
        {
            ...
            if (!conInfo.Reconnecting && !client.Manager.Database.AcquireLock(acc))
            {
                ...
                var otherClients = client.Manager.Clients.Keys
                    .Where(c => c == client || c.Account != null && c.State != ProtocolState.Reconnecting &&
                        (c.Account.AccountId == acc.AccountId ||
                         c.Account.DiscordId != null && c.Account.DiscordId == acc.DiscordId));
                ...
                if (!client.Manager.Database.AcquireLock(acc))
                {
                    client.SendFailure("Account in Use (" +
                                       client.Manager.Database.GetLockTime(acc)?.ToString("%s") +
            ...
            if (world is Test && !(world as Test).JsonLoaded)
            {
                ...
                var dreamName = client.Account.Name.ToLower().EndsWith("s")
                    ? client.Account.Name + "' Dream World"
                    : client.Account.Name + "'s Dream World";
